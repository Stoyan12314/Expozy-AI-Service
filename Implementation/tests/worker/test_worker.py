...
@pytest.mark.asyncio
async def test_marks_retryable_when_a_page_fails(site_generator, mock_page_gen):
    # Arrange
    mock_page_gen.generate_with_retries = AsyncMock(
        side_effect=lambda prompt, page_type, **kwargs: make_failure_result(page_type, errors=["STR-001"], retryable=True)
    )

    # Act
    result = await site_generator.generate("A café")
